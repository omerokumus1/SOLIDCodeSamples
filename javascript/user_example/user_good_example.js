 ...
 *  Here, we decompose the BadUser class into a simple data holder (User class) and separate,
 ...
 *
 *  User (class): Changes only if the core attributes of a user (e.g., adding phoneNumber) change.
...
// Simple class representing a User - now it ONLY holds data.
...
// This class handles saving and retrieving user data.
...
class InMemoryUserRepository {
  ...
  findById(userId) {
    console.log(
      `UserRepository: Finding user by ID: ${userId} from in-memory store.`
    );
...
// This class handles validating user data against business rules.
...
// This class handles formatting user data for various display purposes.
// It would change only if the presentation format or UI requirements change.
class UserPresenter {
  formatForConsole(user) {
    console.log(
...
class UserService {
  constructor(userRepository, userValidator, userPresenter) {
  ...
  createUser(id, name, email) {
    const newUser = new GoodUser(id, name, email);

    // Delegate validation
    this.userValidator.validate(newUser); // Will throw if invalid

    // Delegate saving
    return this.userRepository.save(newUser);
  }

  getFormattedUserDetails(userId, format = 'console') {
